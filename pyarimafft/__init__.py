...
class model:
    ...
    def outlier_clean(
        ...
        model_smooth = sm.nonparametric.lowess(
            endog, list(range(len(endog))), frac=lowess_frac
        )[
            :, 1
        ]  ####trend on actuals.
        ...
        if both_sides_clean == True or (peak_clean == True and trough_clean == True):
            ...
            while count < endog.shape[0]:
                ...
                count = count + 1
        model_smooth = sm.nonparametric.lowess(
            cleaned, list(range(len(endog))), frac=lowess_frac
        )[:, 1]
    ...
    def extract_key_seasonalities(self, power_quantile, time_period=0.01):
        ...
        self.isolated_components = fft_df.iloc[1:neg_index][
            ["K", "COEF", "TIME_PERIOD", "KEY_FREQ"]
        ]
        self.isolated_components = self.isolated_components[
            (self.isolated_components["KEY_FREQ"] == 1)
        ]
        self.isolated_seasonality = np.array(
            self.isolated_components["TIME_PERIOD"].tolist()
        )
        self.isolated_components = fft_df.iloc[0:][
            ["K", "COEF", "TIME_PERIOD", "KEY_FREQ"]
        ]
        self.isolated_components["KEY_COEF"] = (
            self.isolated_components["COEF"] * self.isolated_components["KEY_FREQ"]
        )

    def reconstruct_seasonal_features(self, mode="seperate"):
        time_train = np.arange(self.isolated_components.shape[0])
        time_future = np.arange(
            self.isolated_components.shape[0],
            self.isolated_components.shape[0] + self.forecast_horizon,
        )
        self.time_train = time_train
        self.time_future = time_future
        if mode == "additive":
            self.seasonal_feature_train = np.fft.ifft(
                self.isolated_components["KEY_COEF"]
            )
            self.seasonal_feature_train = self.seasonal_feature_train.real
            key_coeffs = self.isolated_components.iloc[0 : self.neg_index][
                ["K", "KEY_COEF", "KEY_FREQ"]
            ]
            ...
            for i, j in key_coeffs.iterrows():
                complex_wave_pos = np.exp(
                    1j * omega * key_coeffs.loc[i, "K"] * self.time_future
                )
                signal_val_pos = key_coeffs.loc[i, "KEY_COEF"] * complex_wave_pos
                signal_val_pos = signal_val_pos.real
                complex_wave_neg = np.exp(
                    1j * omega * key_coeffs.loc[i, "K"] * self.time_future * -1
                )
                signal_val_neg = (
                    key_coeffs.loc[i, "KEY_COEF"].conjugate() * complex_wave_neg
                )
            ...
            seasonal_feature_future = (
                seasonal_feature_future / self.isolated_components.shape[0]
            )
            seasonal_feature_future = np.sum(seasonal_feature_future, axis=0)
            self.seasonal_feature_future = seasonal_feature_future
            plt.plot(
                self.time_train, self.seasonal_feature_train, label="Train Features"
            )
            plt.plot(
                self.time_future,
                self.seasonal_feature_future,
                label="Features for forecast",
            )
            plt.legend()
            plt.show()
        ...
        if mode == "seperate":
            ...
            for i, j in key_coeffs.iterrows():
                complex_wave_pos = np.exp(
                    1j * omega * key_coeffs.loc[i, "K"] * self.time_train
                )
                signal_val_pos = key_coeffs.loc[i, "KEY_COEF"] * complex_wave_pos
                signal_val_pos = signal_val_pos.real
                complex_wave_neg = np.exp(
                    1j * omega * key_coeffs.loc[i, "K"] * self.time_train * -1
                )
                signal_val_neg = (
                    key_coeffs.loc[i, "KEY_COEF"].conjugate() * complex_wave_neg
                )
            ...
            seasonal_feature_train = (
                seasonal_feature_train / self.isolated_components.shape[0]
            )
            ...
            for i in range(key_coeffs.shape[0]):
                ...
                plt.title(
                    "Time Period {}".format(round(self.isolated_seasonality[i], 2))
                )
                plt.plot(
                    self.time_train,
                    self.seasonal_feature_train[i, :],
                    label="Train Features",
                )
                plt.plot(
                    self.time_future,
                    self.seasonal_feature_future[i, :],
                    label="Features for forecast",
                )
                plt.legend()
    ...
    def add_exog(self, exog):
        ...
        exog_future = exog[
            self.endog.shape[0] : self.endog.shape[0] + self.forecast_horizon
        ]
        ...
        try:
            if (
                self.seasonal_feature_train == None
                and self.seasonal_feature_future == None
            ):
                self.seasonal_feature_train = exog_train
                self.seasonal_feature_future = exog_future
        except:
            self.seasonal_feature_train = np.concatenate(
                [self.seasonal_feature_train, exog_train], axis=0
            )
            self.seasonal_feature_future = np.concatenate(
                [self.seasonal_feature_future, exog_future], axis=0
            )

    def auto_arima(
        self, p=None, d=None, q=None, max_p=3, max_q=3, max_d=1, auto_fit=True
    ):
        if p != None and q != None and d != None:
            if len(self.outlier_cleaned) == 0:
                forecast_model = ARIMA(
                    endog=self.endog,
                    exog=self.seasonal_feature_train.T,
                    order=(p, d, q),
                )
            elif len(self.outlier_cleaned) > 0:
                forecast_model = ARIMA(
                    endog=self.outlier_cleaned,
                    exog=self.seasonal_feature_train.T,
                    order=(p, d, q),
                )
            model_fit = forecast_model.fit()
            preds = model_fit.predict(
                exog=self.seasonal_feature_future.T,
                start=self.endog.shape[0],
                end=self.endog.shape[0] + self.forecast_horizon - 1,
            )
            self.forecast = preds
            plt.plot(range(0, self.endog.shape[0]), self.endog, label="endog")
            plt.plot(
                range(0, self.endog.shape[0]),
                self.outlier_cleaned,
                label="outlier cleaned",
            )
            plt.plot(
                range(self.endog.shape[0], self.endog.shape[0] + self.forecast_horizon),
                preds,
                label="prediction",
            )
            plt.legend()
    ...
    def exponential_smoothing(
        self, trend, damped_trend, seasonal, seasonal_periods, use_boxcox
    ):
        if len(self.outlier_cleaned) == 0:
        ...
        plt.plot(range(0, self.endog.shape[0]), self.endog, label="endog")
        plt.plot(
            range(0, self.endog.shape[0]), self.outlier_cleaned, label="outlier cleaned"
        )
        plt.plot(
            range(self.endog.shape[0], self.endog.shape[0] + self.forecast_horizon),
            preds,
